...
def _generar_pdf_reportlab(lexico, sintactico, semantico, sql) -> bytes:
    ...
    story.append(Paragraph(f"Total tokens: {lexico.get('total_tokens', 0)} | "
                            f"Estado: {'OK - Válido' if lexico['valido'] else 'ERROR - Con errores'}",
                            normal_style))
    ...
    story.append(Paragraph('Fase 2 - Análisis Sintáctico', sub_style))
    story.append(Paragraph(
        f"Estado: {'OK - Válido' if sintactico.get('valido') else 'ERROR - Con errores'}",
        normal_style))
    for e in sintactico.get('errores', []):
        story.append(Paragraph(f"! {e}", normal_style))
    story.append(Spacer(1, 0.2*cm))
    prod_data = [['Producción']] + [[p] for p in sintactico.get('producciones', [])]
    t_prod = Table(prod_data, colWidths=[16*cm])
    ...
    for r in semantico.get('reglas_aplicadas', []):
        reglas_data.append([r['regla'], r['descripcion'], _estado_texto(r['estado']), r['valor']])
    t3 = Table(reglas_data, colWidths=[2*cm, 7*cm, 2*cm, 7*cm])
    estado_style = [('TEXTCOLOR', (2, i), (2, i), VERDE if fila[2] == 'OK' else ROJO)
                    for i, fila in enumerate(reglas_data[1:], start=1)]
    ...
    sql_texto = '\n'.join(sql.split('\n')[:60]) or ' '
    story.append(Preformatted(sql_texto, sql_style))
...
def _generar_pdf_html(lexico, sintactico, semantico, sql) -> str:
    ...
    html += f"""</table>
    <h2>Fase 2 — Análisis Sintáctico</h2>
    <p>Estado: <span class="{'ok' if sintactico.get('valido') else 'err'}">
    {'✅ Válido' if sintactico.get('valido') else '❌ Con errores'}</span></p>
    """
    for e in sintactico.get('errores', []):
        html += f'<p class="err">⚠ {e}</p>\n'
    html += """<table>
    <tr><th>Producción</th></tr>
    """
    for p in sintactico.get('producciones', []):
